...
import com.sehaj.wakeme.AlarmPreference.Type;
...
public class AlarmPreferenceListAdapter extends BaseAdapter {
	...
	public AlarmPreferenceListAdapter(Context context, Alarm alarm) {
		...
		if (alarmsCursor.moveToFirst()) {		    			
			do {
				alarmTones[alarmsCursor.getPosition()+1] = ringtoneManager.getRingtone(alarmsCursor.getPosition()).getTitle(getContext());
				alarmTonePaths[alarmsCursor.getPosition()+1] = ringtoneManager.getRingtoneUri(alarmsCursor.getPosition()).toString();
		...
		for(AlarmPreference preference : preferences){
			switch(preference.getKey()){
				...
				case ALARM_REPEAT:
					...
					break;
				case ALARM_SMS_NO:
					alarm.setSmsNo((String) preference.getValue());
					break;
				case ALARM_SMS_MESSAGE:
					alarm.setSmsMessage((String) preference.getValue());
					break;
			}
		}
	...
	public void setAlarm(Alarm alarm) {
		...
		preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_SMS_NO, "Distress Sms Number",alarm.getSmsNo(), null, alarm.getSmsNo(), Type.STRING));
		preferences.add(new AlarmPreference(AlarmPreference.Key.ALARM_SMS_MESSAGE, "Distress Sms Message",alarm.getSmsMessage(), null, alarm.getSmsMessage(), Type.STRING));
	}
